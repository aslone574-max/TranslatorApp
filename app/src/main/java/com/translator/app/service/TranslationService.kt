...
class TranslationService(private val context: Context) {
    ...
    suspend fun translateAudio(audioFile: File, targetLanguage: Language): TranslationResult =
        withContext(Dispatchers.IO) {
            ...
            if (isOnline() && apiKey.isNotEmpty()) translateAudioGroq(audioFile, targetLanguage, apiKey)
            else throw Exception("음성 번역은 SpeechService를 통해 처리됩니다.")
        }

    suspend fun translateText(text: String, targetLanguage: Language): TranslationResult =
        withContext(Dispatchers.IO) {
            val apiKey = getApiKey()
            if (isOnline() && apiKey.isNotEmpty()) translateTextGroq(text, targetLanguage, apiKey)
            else translateTextMlKit(text, targetLanguage)
        }

    private suspend fun translateAudioGroq(audioFile: File, targetLanguage: Language, apiKey: String): TranslationResult =
        withContext(Dispatchers.IO) {
            val auth = "Bearer $apiKey"
            val audioPart = MultipartBody.Part.createFormData("file", audioFile.name, audioFile.asRequestBody("audio/m4a".toMediaType()))
            val whisperResult = NetworkClient.api.transcribe(auth, audioPart, "whisper-large-v3".toRequestBody("text/plain".toMediaType()), "verbose_json".toRequestBody("text/plain".toMediaType()), null)
    ...
    private suspend fun translateTextGroq(text: String, targetLanguage: Language, apiKey: String): TranslationResult =
        withContext(Dispatchers.IO) {
            val auth = "Bearer $apiKey"
    ...
    private suspend fun groqChat(auth: String, text: String, from: Language, to: Language): String {
        ...
        val response = NetworkClient.api.chat(auth, ChatRequest(messages = listOf(ChatMsg("system", system), ChatMsg("user", text))))
        return response.choices.firstOrNull()?.message?.content?.trim() ?: throw Exception("번역 결과를 받지 못했습니다.")
    }
    ...
    suspend fun translateTextMlKit(text: String, targetLanguage: Language): TranslationResult =
        withContext(Dispatchers.IO) {
            ...
            val options = TranslatorOptions.Builder().setSourceLanguage(fromCode).setTargetLanguage(toCode).build()
    ...
    private fun containsKorean(text: String) = text.any { it in '\uAC00'..'\uD7A3' || it in '\u1100'..'\u11FF' }
